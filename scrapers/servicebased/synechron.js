...
class SynechronJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Synechron Careers...');
        await this.page.goto('https://www.synechron.com/careers/jobs', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(
                'div[data-dynamic-filter-location] a',
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                return {
                    title: getText('h2.css-7papts'),
                    company: 'Synechron',
                    location: getText('dd.css-129m7dg'),
                    description: getText('div.css-148wyr7'),
...
const runSynechronJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new SynechronJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runSynechronJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runSynechronJobsScraper({ headless: headlessArg });
    })();
}
