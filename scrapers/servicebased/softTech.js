...
class SoftTechJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Softtech Engineers Ltd Careers...');
        await this.page.goto('https://softtechglobal.com/careers/', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval("li.rec-job-title a", anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                return {
                    title: getText('h1[style="color:#FFFFFF"]'),
                    company: 'Softtech Engineers Ltd',
                    location: getText('p[style="color:#FFFFFF"]'),
                    description: getText('#cw-rich-description'),
...
const runSoftTechJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new SoftTechJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runSoftTechJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runSoftTechJobsScraper({ headless: headlessArg });
    })();
}
