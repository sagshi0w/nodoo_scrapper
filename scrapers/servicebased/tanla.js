...
class TanlaJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Tanla Careers...');
        await this.page.goto('https://www.tanla.com/careers/jobs-listing', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(
                '.job-list_collection-item-btn-wrapper link[rel="prefetch"]',
...
const runTanlaJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new TanlaJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runTanlaJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runTanlaJobsScraper({ headless: headlessArg });
    })();
}
