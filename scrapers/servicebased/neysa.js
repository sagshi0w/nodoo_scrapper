...
class NeysaJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Neysa Careers...');
        await this.page.goto('https://neysa.ai/careers/job-openings/', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(`div.jd-btn-wrapper > a.job-btn`, anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                let rawTitle = getText('h1.job-title');
...
const Neysa = async ({ headless = true } = {}) => {
    const scraper = new NeysaJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};


export default Neysa;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await Neysa({ headless: headlessArg });
    })();
}
