...
class ustGlobalJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to UST Global Careers...');
        await this.page.goto('https://usource.ripplehire.com/candidate/?token=xHQWoFn4C242POo7xMpH&source=CAREERSITE#list/location=Ahmedabad%2BBangalore%2BChennai%2BGurgaon%2BGurugram%2BHyderabad%2BKochi%2BKolkata%2BMumbai%2BNavi%20Mumbai%2BPune%2BThane%2BThiruvananthapuram%2BTrivandrum', {
...
const runUstGlobalJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new ustGlobalJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runUstGlobalJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runUstGlobalJobsScraper({ headless: headlessArg });
    })();
}
