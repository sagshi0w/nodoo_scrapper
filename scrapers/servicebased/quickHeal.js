...
class QuickHealJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Quick Heal Careers...');
        await this.page.goto('https://lifecycleqhtl.darwinbox.in/ms/candidate/careers', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval("a.clickable.color-blue.custom-theme-color", anchors =>
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                return {
                    ...
                    description: getHtml('div.job-summary') || '',
...
const extractWiproData = (job) => {
    ...
    if (cleanedDescription) {
        // Remove unwanted headers like Group Company, Designation, Location, etc.
        cleanedDescription = cleanedDescription.replace(
            /(Group Company:|Designation:|Office Location:|Years of experience:|Salary Range:)\s*.*(?:\n|$)/gi,
            ''
        );
...
const runQuickHealJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new QuickHealJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runQuickHealJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runQuickHealJobsScraper({ headless: headlessArg });
    })();
}
