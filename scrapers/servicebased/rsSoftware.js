...
class RSSoftwareJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to RS Software Careers...');
        await this.page.goto('https://www.rssoftware.com/home/jointeam', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval("a.job-more", anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                let rawTitle = getText('h2');
                let title = rawTitle.trim();

                return {
                    title,
                    company: 'RS Software',
                    description: getText('div.post-content'),
...
const RSSoftware = async ({ headless = true } = {}) => {
    const scraper = new RSSoftwareJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default RSSoftware;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await RSSoftware({ headless: headlessArg });
    })();
}
