...
class RamcoSystemsJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Ramco Systems Careers...');
        await this.page.goto('https://www.ramco.com/careers/jobs-by-locations', {
...
const runRamcoSystemsJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new RamcoSystemsJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runRamcoSystemsJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runRamcoSystemsJobsScraper({ headless: headlessArg });
    })();
}
