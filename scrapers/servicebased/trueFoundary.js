...
class TruefoundaryJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to TrueFoundry Careers...');
        await this.page.goto('https://app.careerpuck.com/job-board/truefoundry', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(`a[data-testing="job-row"]`, anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                let rawTitle = getText('h1 > div.role');
                let title = rawTitle.trim();

                return {
                    title,
                    company: 'TrueFoundry',
                    location: getText('div.JobPagePublic__Location-sc-tfpwjw-4'),
...
const TrueFoundary = async ({ headless = true } = {}) => {
    const scraper = new TruefoundaryJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};


export default TrueFoundary;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await TrueFoundary({ headless: headlessArg });
    })();
}
