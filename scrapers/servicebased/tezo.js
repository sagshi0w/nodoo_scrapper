...
class TezoJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Tezo Careers...');
        await this.page.goto('https://www.tezo.com/careers', {
...
const runTezoJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new TezoJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runTezoJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runTezoJobsScraper({ headless: headlessArg });
    })();
}
