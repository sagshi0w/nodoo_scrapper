...
class infiniteComputerSolutionsJobsScraper {
...
const runInfiniteComputerSolutionsJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new infiniteComputerSolutionsJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runInfiniteComputerSolutionsJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runInfiniteComputerSolutionsJobsScraper({ headless: headlessArg });
    })();
}
