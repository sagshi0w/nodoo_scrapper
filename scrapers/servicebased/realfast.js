...
class RealfastJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Realfast Careers...');
        await this.page.goto('https://jobs.ashbyhq.com/realfast', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(`a._container_j2da7_1`, anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                let rawTitle = getText('div._titles_ud4nd_34 h1._title_ud4nd_34');
                let title = rawTitle.trim();

                return {
                    title,
                    company: 'Realfast',
                    location: getText('div._section_101oc_37 p'),
...
const Realfast = async ({ headless = true } = {}) => {
    const scraper = new RealfastJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};


export default Realfast;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await Realfast({ headless: headlessArg });
    })();
}
