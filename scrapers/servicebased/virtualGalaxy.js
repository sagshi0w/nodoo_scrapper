...
class VirtualGalaxyJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Virtual Galaxy Careers...');
        await this.page.goto('https://www.vgipl.com/careers', {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                gridItems.forEach(item => {
                    const label = item.querySelector('p')?.innerText.trim().toLowerCase();
                    const value = item.querySelector('h4')?.innerText.trim();
...
const runVirtualGalaxyJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new VirtualGalaxyJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runVirtualGalaxyJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runVirtualGalaxyJobsScraper({ headless: headlessArg });
    })();
}
