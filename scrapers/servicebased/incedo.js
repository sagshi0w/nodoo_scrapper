...
class IncodeJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Incedo Careers...');
        await this.page.goto('https://career44.sapsf.com/career?company=incedotech&career%5fns=job%5flisting%5fsummary&navBarLevel=JOB%5fSEARCH&_s.crb=853bZWqeAEMCh9AHRh0JmqTDN2EbSTK3CYRywyu8qy4%3d', {
...
const runIncodeJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new IncodeJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runIncodeJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runIncodeJobsScraper({ headless: headlessArg });
    })();
}
