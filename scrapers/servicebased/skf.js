...
const cleanJobDescription = (description, company = 'SKF') => {
    ...
    cleaned = cleaned
        ...
        .replace(/^[^.]*SKF is on a mission[^.]*\.\s*/gi, '')
        ...
        // Remove SKF-specific boilerplate
        .replace(/About\s+SKF\s*/gi, '')
        .replace(/SKF started its operations in India[\s\S]*?business excellence\./i, '')
        .replace(/SKF's solutions provide sustainable ways[\s\S]*?five technology platforms\./i, '')
        .replace(/To know more, please visit:\s*[^\n]+/i, '')
        .replace(/www\.skf\.com\/in/gi, '')

        // Remove alternate About SKF paragraph and See more lines
        .replace(/About\s+SKF\s*/gi, '')
        .replace(/SKF\s+has\s+been\s+around\s+for\s+more\s+than\s+a\s+century[\s\S]*?(?:See\s+more,\s*at\s*[^\n]+\.?|$)/i, '')
        .replace(/See\s*more,\s*at\s*[^\n]+/i, '')
        .replace(/www\.skf\.com/gi, '')
        
        // Remove "About the job" section
        .replace(/About the job\s*/gi, '')

        // Remove About Technology Development section and TD Competencies block
        .replace(/About\s+Technology\s+Development[\s\S]*?(?=TD\s+Competencies|SKF\s+Purpose\s+Statement|Role\s+Purpose|Key\s+Responsibilities|$)/gi, '')
        .replace(/TD\s+Competencies[\s\S]*?(?=SKF\s+Purpose\s+Statement|Role\s+Purpose|Key\s+Responsibilities|JOB\s+DESCRIPTION|$)/gi, '')

        // Remove SKF Purpose Statement and its lines
        .replace(/SKF\s+Purpose\s+Statement[\s\S]*?(?=JOB\s+DESCRIPTION|Role\s+Purpose|Key\s+Responsibilities|$)/gi, '')
        .replace(/Together,\s*we\s*re-imagine\s*rotation\s*for\s*a\s*better\s*tomorrow\.?/gi, '')
        .replace(/By\s*creating\s*intelligent\s*and\s*clean\s*solutions\s*for\s*people\s*and\s*the\s*planet\.?/gi, '')

        // Remove label lines like Job Title, Reports To, Role Type, Location
        .replace(/^\s*(?:Job\s*Title|Reports\s*To|Role\s*Type|Location)\s*:\s*.*$/gim, '')
