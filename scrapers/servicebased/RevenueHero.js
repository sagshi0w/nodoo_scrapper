...
class RevenueHeroJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to RevenueHero Careers...');
        await this.page.goto('https://www.revenuehero.io/careers', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(`.rec-job-title a`, anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                // Extract job title
                let rawTitle = getText('.cw-jobheader-info h1');
                let title = rawTitle.trim();

                // Extract Location and Experience from Job Information section
                ...
                const description = getText('div#cw-rich-description');
        ...
        for (let i = 0; i < this.allJobLinks.length; i++) {
            ...
            if (jobData && jobData.title) {
                ...
                if (jobData.title.toLowerCase() === "open roles") {
                    ...
                    const enrichedJob = extractRevenueHeroData(jobData);
...
const extractRevenueHeroData = (job) => {
    if (!job) return job;
    ...
    if (cleanedDescription) {
        // Remove sections like Job Title, Location, Type, Experience, About Us intro, Why Join Us, About RevenueHero
...
const RevenueHero = async ({ headless = true } = {}) => {
    const scraper = new RevenueHeroJobsScraper(headless);
    await scraper.run();
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    (async () => {
        await RevenueHero({ headless: headlessArg });
    })();
}
