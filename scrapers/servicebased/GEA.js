...
class GEAJobsScraper {
    ...
    async navigateToJobsPage() {
        ...
        try {
            ...
            await this.page.waitForFunction(
                () => {
                    return document.querySelectorAll('a[data-testid="careers-listing-row"]').length > 0;
                },
                { timeout: 15000 }
            ).catch(() => {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            // Wait for job links to load using a[data-testid="careers-listing-row"] selector
            try {
                await this.page.waitForSelector('a[data-testid="careers-listing-row"]', { timeout: 15000 });
            } catch (err) {
                console.log('✅ No more job links found. Done.');
                break;
            }

            // Collect new links using a[data-testid="careers-listing-row"] selector
            let jobLinks = [];
            try {
                jobLinks = await this.page.$$eval(
                    'a[data-testid="careers-listing-row"]',
...
const runGEAJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new GEAJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runGEAJobsScraper;

// ✅ CLI support: node GEA.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runGEAJobsScraper({ headless: headlessArg });
    })();
}
