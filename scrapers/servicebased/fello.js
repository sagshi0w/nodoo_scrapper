...
class FelloJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Fello Careers...');
        await this.page.goto('https://fello.ai/careers#job-listings', {
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                // Extract job title
                let rawTitle = getText('h2.css-10i6wsd-H2Element');
                ...
                const description = getText('div.ATS_htmlPreview') || getText('div#cw-rich-description');
        ...
        for (let i = 0; i < this.allJobLinks.length; i++) {
            ...
            if (jobData && jobData.title) {
                ...
                if (jobData.title.toLowerCase() === "open roles") {
                    ...
                    const enrichedJob = extractFelloData(jobData);
...
const Fello = async ({ headless = true } = {}) => {
    const scraper = new FelloJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};


export default Fello;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await Fello({ headless: headlessArg });
    })();
}
