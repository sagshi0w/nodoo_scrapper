...
class SarvamAIJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Sarvam AI Careers...');
        await this.page.goto('https://careers.kula.ai/sarvam-ai', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(`a.chakra-link.css-i3x6xc[href^="/sarvam-ai/"]`, anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                let rawTitle = getText('div.css-1zb6fj > p.chakra-text.css-1xznk5k');
                let title = rawTitle.trim();

                return {
                    title,
                    company: 'Sarvam AI',
                    location: getText('div.css-1zb6fj > p.chakra-text.css-74e7yv'),
                    description: getText('div.css-1vav6o2'),
...
const SarvamAI = async ({ headless = true } = {}) => {
    const scraper = new SarvamAIJobsScraper(headless);
    await scraper.run();
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    (async () => {
        await SarvamAI({ headless: headlessArg });
    })();
}
