...
class WNSJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to WNS Careers...');
        await this.page.goto('https://careers.smartrecruiters.com/WNSGlobalServices144/vuram-career-page', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(
                'li.opening-job > a.link--block.details',
...
const runWNSJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new WNSJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runWNSJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runWNSJobsScraper({ headless: headlessArg });
    })();
}
