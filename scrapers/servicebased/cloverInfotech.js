...
class CLoverInfotechJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Clover Infotech Careers...');
        await this.page.goto('https://www.cloverinfotech.com/job-openings/', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(
                '.article--career__wrap a[href^="https://www.cloverinfotech.com/jobs/"]',
...
const runCLoverInfotechJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new CLoverInfotechJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runCLoverInfotechJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runCLoverInfotechJobsScraper({ headless: headlessArg });
    })();
}
