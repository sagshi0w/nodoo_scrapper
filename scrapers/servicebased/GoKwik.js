...
class GoKwikJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to GoKwik Careers...');
        await this.page.goto('https://gokwik.keka.com/careers/', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(`a.kh-card.kh-job-card.kh-job-container`, anchors =>
...
const GoKwik = async ({ headless = true } = {}) => {
    const scraper = new GoKwikJobsScraper(headless);
    await scraper.run();
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    (async () => {
        await GoKwik({ headless: headlessArg });
    })();
}
