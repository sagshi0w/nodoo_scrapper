...
class CoreworksJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Coreworks Careers...');
        await this.page.goto('https://www.coreworks.ai/', {
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                // Extract job title
                let rawTitle = getText('h1[contenteditable="false"]');
                let title = rawTitle.trim();

                // Extract Location
                const locationElement = Array.from(document.querySelectorAll('div[data-block-id] div[contenteditable="false"]'))
                    .find(el => el.innerText.startsWith('Location:'));
                const locationText = locationElement ? locationElement.innerText.trim() : '';
                const location = locationText.replace(/^Location:\s*/, '').trim() || 'Bangalore';

                // Extract Experience
                const experienceElement = Array.from(document.querySelectorAll('div[data-block-id] div[contenteditable="false"]'))
                    .find(el => el.innerText.startsWith('Experience:'));
                const experienceText = experienceElement ? experienceElement.innerText.trim() : '';
...
const Coreworks = async ({ headless = true } = {}) => {
    const scraper = new CoreworksJobsScraper(headless);
    await scraper.run();
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    (async () => {
        await Coreworks({ headless: headlessArg });
    })();
}
