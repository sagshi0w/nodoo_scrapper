...
class SpendfloJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Spendflo Careers...');
        await this.page.goto('https://spendflo.zohorecruit.com/jobs/Job-openings', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(`a.cw-3-title.cw-bw`, anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                const description = getText('div#cw-rich-description span#spandesc');
...
const Spendflo = async ({ headless = true } = {}) => {
    const scraper = new SpendfloJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};


export default Spendflo;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await Spendflo({ headless: headlessArg });
    })();
}
