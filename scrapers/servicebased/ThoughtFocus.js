...
const runThoughtFocusJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new ThoughtFocusJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runThoughtFocusJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runThoughtFocusJobsScraper({ headless: headlessArg });
    })();
}
