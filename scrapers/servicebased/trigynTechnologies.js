...
class TrigynTechnologiesJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Trigyn Technologies Careers...');
        await this.page.goto('https://www.trigyn.com/careers', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval("a[title][aria-label]", anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                let rawTitle = getText('h1.entry-title');
                let title = rawTitle.replace(/^Job Details:\s*/i, '').trim();

                return {
                    title,
                    company: 'Trigyn Technologies',
                    location: getText('.job_field_col_2 > div:nth-child(1) > span.job_field_value'),
                    description: getText('.column-content-inner .column-content'),
...
const runTrigynTechnologiesJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new TrigynTechnologiesJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runTrigynTechnologiesJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runTrigynTechnologiesJobsScraper({ headless: headlessArg });
    })();
}
