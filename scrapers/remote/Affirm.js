...
class AffirmJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Affirm Careers...');
        await this.page.goto('https://www.affirm.com/careers#jobOpenings', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval('a[data-testid="jobCard"]', anchors =>
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                let rawTitle = getText('.job__title h1.section-header, h1.wp-block-post-title.has-text-align-center.is-typography-preset-h1, h1.wp-block-post-title, div.job-title-container h1.job-title, h1.job-title');
        ...
        for (let i = 0; i < this.allJobLinks.length; i++) {
            ...
            if (jobData && jobData.title) {
                ...
                if (jobData.title.toLowerCase() === "open roles") {
                    ...
                    const enrichedJob = extractAffirmData(jobData);
...
const Affirm = async ({ headless = true } = {}) => {
    const scraper = new AffirmJobsScraper(headless);
    await scraper.run();
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    (async () => {
        await Affirm({ headless: headlessArg });
    })();
}
