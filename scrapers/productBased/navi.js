...
class NaviJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Navi Careers...');
        await this.page.goto('https://navi.freshteam.com/jobs', {
...
const runNaviJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new NaviJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runNaviJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runNaviJobsScraper({ headless: headlessArg });
    })();
}
