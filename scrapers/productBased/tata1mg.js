...
class TatamgJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to TATA MG Careers...');
        await this.page.goto('https://1mg.darwinbox.in/ms/candidatev2/main/careers/allJobs', {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            ...
            const jobLinks = await this.page.$$eval(
                'a.db-btn.style-secondary.action-btn',
    ...
    async extractJobDetailsFromLink(url) {
        ...
        try {
            ...
            const job = await jobPage.evaluate(() => {
                ...
                return {
                    title: getText('div.title-section > span.title'),
                    company: 'TATA 1mg',
                    location: getText('div.title-section div.sub-title div.sub-section:nth-child(1) > span'),
                    description: getText('div.jd'),
...
const runTatamgJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new TatamgJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runTatamgJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runTatamgJobsScraper({ headless: headlessArg });
    })();
}
