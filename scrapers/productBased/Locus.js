...
class LocusJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Locus Careers...');
        await this.page.goto('https://locus.freshteam.com/jobs', {
...
const runLocusJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new LocusJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runLocusJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runLocusJobsScraper({ headless: headlessArg });
    })();
}
