...
class FreechargeJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Freecharge Careers...');
        await this.page.goto('https://freecharge.ripplehire.com/candidate/?token=IoV5vvUSMKLwmaa1Suou&source=CAREERSITE#list', {
...
const runFreechargeJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new FreechargeJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runFreechargeJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runFreechargeJobsScraper({ headless: headlessArg });
    })();
}
