...
class AdobeJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Adobe Careers...');
        try {
            ...
            await this.page.waitForFunction(
                () => {
                    return document.querySelectorAll('a[data-ph-at-id="job-link"]').length > 0 ||
                           document.body.innerText.includes('Apply') ||
                           document.body.innerText.includes('Job');
                },
                { timeout: 15000 }
            ).catch(() => {
    ...
    async collectAllJobCardLinks() {
        ...
        while (true) {
            // Wait for job links to load using the data-ph-at-id selector
            try {
                await this.page.waitForSelector('a[data-ph-at-id="job-link"]', { timeout: 15000 });
            } catch (err) {
            ...
            // Collect new links using a[data-ph-at-id="job-link"] selector
            let jobLinks = [];
            try {
...
const runAdobeJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new AdobeJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runAdobeJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runAdobeJobsScraper({ headless: headlessArg });
    })();
}
