...
class TextBookJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to TextBook Careers...');
        await this.page.goto('https://testbook.hire.trakstar.com/', {
...
const runTextBookJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new TextBookJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runTextBookJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runTextBookJobsScraper({ headless: headlessArg });
    })();
}
