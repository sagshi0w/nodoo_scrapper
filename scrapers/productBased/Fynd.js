...
class FyndJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Fynd Careers...');
        await this.page.goto('https://www.fynd.com/careers#open-positions', {
...
const runFyndJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new FyndJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runFyndJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runFyndJobsScraper({ headless: headlessArg });
    })();
}
