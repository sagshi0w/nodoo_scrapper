...
class KhanAcademyIndiaJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Khan Academy India Careers...');
        await this.page.goto('https://hi.khanacademy.org/careers#openings', {
...
const runKhanAcademyIndiaJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new KhanAcademyIndiaJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runKhanAcademyIndiaJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runKhanAcademyIndiaJobsScraper({ headless: headlessArg });
    })();
}
