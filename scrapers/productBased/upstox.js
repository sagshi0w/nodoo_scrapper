...
class UpstoxJobsScraper {
    ...
    async navigateToJobsPage() {
        console.log('🌐 Navigating to Upstox Careers...');
        await this.page.goto('https://jobs.lever.co/upstox', {
...
const runUpstoxJobsScraper = async ({ headless = true } = {}) => {
    const scraper = new UpstoxJobsScraper(headless);
    await scraper.run();
    return scraper.allJobs;
};

export default runUpstoxJobsScraper;

// ✅ CLI support: node phonepe.js --headless=false
if (import.meta.url === `file://${process.argv[1]}`) {
    const headlessArg = process.argv.includes('--headless=false') ? false : true;
    (async () => {
        await runUpstoxJobsScraper({ headless: headlessArg });
    })();
}
